...
class TunnelManager:
    ...
    def _ensure_binary(self):
        ...
        # Multi-source download URLs (Official + Mirrors)
        urls = [
        ...
        for url in urls:
            ...
            try:
                # Prioritize curl (-k ignores SSL issues, -L follows redirects)
                # Use check=True to ensure errors are caught
    ...
    def start(self, server_addr, server_port, token, remote_port=None, local_port=8188, subdomain=None):
        # [FIX] Clean up server_addr in case user included protocol or trailing slashes
        ...
        # [SECURITY] Never print the config content as it contains the sensitive token
        # print(f"[ComfyPanel] Config Content:\n{config_content}")
    ...
    def stop(self):
        if self._process:
            ...
            # [SECURITY] Cleanup temporary config file
    ...
    def _monitor_logs(self):
        ...
        try:
            for line in iter(self._process.stdout.readline, ''):
                ...
                # Removed general print(f"[frpc] {line_str}") for security and cleanliness
                    
                if "login to server failed" in line_str or "error" in line_str.lower() or "[W]" in line_str or "[E]" in line_str:
                    print(f"[Tunnel ERROR] {line_str}") # Only print errors
